...
_DEBUG = True  # 调试开关：打印上游请求明细，排查完毕后改为 False
...
async def generate_ai_image(prompt, size, quality, model, reference_images=None, provider_id="comfly"):
    ...
    _dbg("开始生图", {
        ...
        "model": model,
        "is_gpt2": is_gpt2,
        "is_apimart": is_apimart,
        "size": size,
        "quality": quality,
        "image_refs_count": len(image_refs),
        "mask_refs_count": len(mask_refs),
        "gen_url": gen_url,
        "edit_url": edit_url,
    ...
    async with httpx.AsyncClient(timeout=request_timeout) as client:
        ...
        async def post_openai_edits(edit_files=None, url=None):
            parts = [
                ("model", (None, model)),
                ("prompt", (None, prompt)),
                ("size", (None, size)),
            ...
            if quality:
                parts.append(("quality", (None, quality)))
            if edit_files:
                parts.extend(edit_files)
            hdrs = api_headers(json_body=False, provider=provider)
            target_url = url or edit_url
            _dbg("EDIT 请求 → POST " + target_url, {
                "headers": {k: v for k, v in hdrs.items() if k.lower() != "authorization"},
                "form_fields": [p[0] for p in parts if p[1][0] is None],
                "file_fields": [p[0] for p in parts if p[1][0] is not None],
            })
            resp = await client.post(target_url, headers=hdrs, files=parts)
            _dbg("EDIT 响应 ← HTTP " + str(resp.status_code), resp.text[:1500] if resp.status_code >= 400 else "(ok)")
            return resp
        ...
        if is_apimart:
            ...
            _dbg("APIMart 请求 → POST " + gen_url, {"body": {**body, "prompt": prompt[:100] + "..." if len(prompt) > 100 else prompt}})
            response = await client.post(gen_url, headers=api_headers(provider=provider), json=body)
            _dbg("APIMart 响应 ← HTTP " + str(response.status_code), response.text[:1500] if response.status_code >= 400 else "(ok)")
        elif is_gpt2 and not image_refs and not mask_refs:
            body = {"model": model, "prompt": prompt, "size": size}
            if quality:
                body["quality"] = quality
            _dbg("GPT2-文生图 请求 → POST " + gen_url, {"body": {**body, "prompt": prompt[:100] + "..." if len(prompt) > 100 else prompt}})
            response = await client.post(gen_url, headers=api_headers(provider=provider), json=body)
            _dbg("GPT2-文生图 响应 ← HTTP " + str(response.status_code), response.text[:1500] if response.status_code >= 400 else "(ok)")
            if response.status_code >= 400 and images_api_unsupported(response):
                response = await post_openai_edits()
        elif image_refs:
            # 图生图主路径：multipart → /v1/images/edits
            # 仅当 edits 明确不支持时才回退到 generations JSON image:[data_url]
            files = []
            opened = []
            try:
                primary_ref = image_refs[0]
                path = output_file_from_url(primary_ref.get("url", ""))
                if not path:
                    raise HTTPException(status_code=400, detail="参考图文件无法读取")
                fh = open(path, "rb")
                opened.append(fh)
                ...
                if response.status_code >= 400 and images_api_unsupported(response):
                    fail_text = response.text[:500]
                    fail_status = response.status_code
                    print(f"/images/edits unsupported ({fail_status}): {fail_text[:200]} → 回退到 generations JSON")
                    ...
                    if quality:
                        body["quality"] = quality
                    _dbg("EDIT回退→generations JSON POST " + gen_url, {
                        "body": {**{k: v for k, v in body.items() if k != "image"}, "image_count": len(image_payload)},
                    })
                    response = await client.post(gen_url, headers=api_headers(provider=provider), json=body)
                    _dbg("EDIT回退→generations 响应 ← HTTP " + str(response.status_code), response.text[:1500] if response.status_code >= 400 else "(ok)")
                    if response.status_code >= 400:
                        raise HTTPException(
                            status_code=502,
                            detail=f"/images/edits 不支持（{fail_text[:200]}），generations JSON 回退也失败（{response.status_code}）：{response.text[:300]}"
                        )
            finally:
                for fh in opened:
                    fh.close()
            ...
            if quality:
                body["quality"] = quality
            _dbg("标准 请求 → POST " + gen_url, {"body": {**body, "prompt": prompt[:100] + "..." if len(prompt) > 100 else prompt}})
            response = await client.post(
                gen_url,
                headers=api_headers(provider=provider),
                json=body,
            )
            _dbg("标准 响应 ← HTTP " + str(response.status_code), response.text[:1500] if response.status_code >= 400 else "(ok)")
            if response.status_code >= 400 and images_api_unsupported(response):
                response = await post_openai_edits()
