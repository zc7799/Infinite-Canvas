...
from upload_utils import (
    AI_REFERENCE_UPLOAD_MAX_BYTES,
    COMFY_UPLOAD_MAX_BYTES,
...
if os.getenv("INFINITE_CANVAS_SKIP_RUNTIME_INIT") != "1":
...
async def upload_image(files: List[UploadFile] = File(...)):
    ...
    files_content = []
    for file in files:
        content = await read_upload_file_limited(file, COMFY_UPLOAD_MAX_BYTES, file.filename or "上传文件")
...
async def upload_ai_reference(files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        size = await save_upload_file_limited(file, path, AI_REFERENCE_UPLOAD_MAX_BYTES, file.filename or "上传文件")
        if size <= 0:
            try:
                os.remove(path)
            except OSError:
